...
// ----- Serial
//LiquidCrystal_I2C Serial(0x27, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE);          // Serial pinouts: addr,en,rw,rs,d4,d5,d6,d7,bl,blpol
int Serial_loop_counter;
int Serial_heading_buffer, Serial_pitch_buffer, Serial_roll_buffer;
...
int     Mag_x_offset = -10,      Mag_y_offset = 122,     Mag_z_offset = -202;   // Hard-iron offsets
float   Mag_x_scale = 1.02,     Mag_y_scale = 0.99,     Mag_z_scale = 1.00;    // Soft-iron scale factors
float   ASAX = 1.19,            ASAY = 1.18,            ASAZ = 1.14;           // (A)sahi (S)ensitivity (A)djustment fuse ROM values.
...
void setup()
{
  ...
  */
  ...
  // ----- Start-up message
  // Clear the Serial
  // Set the Serial cursor to column 0 line 0
  Serial.print(" Tilt Compass");                           // Print text to screen
  // Set the Serial cursor to column 0 line 1
...
void loop()
{
  ...
  */
  ...
  Accel_pitch -= -3.72f;                                             //Accelerometer calibration value for pitch
  Accel_roll -= 1.54f;                                               //Accelerometer calibration value for roll
...
void calibrate_magnetometer()
{
  ...
  for (int counter = 0; counter < 16000 ; counter ++)             // Run this code 16000 times
  {
    ...
    if (counter % 1000 == 0)Serial.print(".");                        // Print a dot on the Serial every 1000 readings
...
void calibrate_gyro()
{
  ...
  {
    Loop_start = micros();
    if (counter % 125 == 0)Serial.print(".");              //Print a dot on the Serial every 125 readings
    read_mpu_6050_data();                               //Read the raw acc and gyro data from the MPU-6050
...
{
  /*
     Subroutine for updating the Serial display.
  ...
  */
  ...
  //      Serial_heading_buffer = round(Heading);                    // Buffer the (true north) heading because it will change
  //                                        // Set the Serial cursor to column 10, row 0
  //      Serial.print("     ");                                     // Clear digits
  //                                        // Set the Serial cursor to column 10, row 0
  //      Serial.print(Serial_heading_buffer);
  //      break;
  //
  //    /////////////////////// Pitch ///////////////////////////
  //    case 2: // ----- Buffer the pitch angle
  //      Serial_pitch_buffer = Gyro_pitch_output * 10;            // Buffer the gyro pitch angle because it will change
  //                                      // Set the Serial cursor to column 10, row 1
  //      break;
  //
  //    case 3: // ----- Display pitch sign
  //      (Serial_pitch_buffer < 0) ? Serial.print("-") : Serial.print("+");
  //      break;
  //
  //    case 4: // ----- Display 1st pitch digit
  //      Serial.print(abs(Serial_pitch_buffer) / 1000);
  //      break;
  //
  //    case 5: // ----- Display 2nd pitch digit
  //      Serial.print((abs(Serial_pitch_buffer) / 100) % 10);
  //      break;
  //
  //    case 6: // ----- Display 3rd pitch digit
  //      Serial.print((abs(Serial_pitch_buffer) / 10) % 10);
  //      break;
  //
  ...
  //      Serial.print(abs(Serial_pitch_buffer) % 10);
  //      break;
  //
  //    ///////////////////// Roll //////////////////////////
  //    case 9: // ----- Buffer pitch reading
  //      Serial_roll_buffer = Gyro_roll_output * 10;          // Buffer the gyro roll angle because it will change
  //                                    // Set the Serial cursor to column 1, row 0
  //      break;
  //
  //    case 10: // ----- Display roll sign
  //      (Serial_roll_buffer < 0) ? Serial.print("-") : Serial.print("+");
  //      break;
  //
  //    case 11: // ----- Display 1st roll digit
  //      Serial.print(abs(Serial_roll_buffer) / 1000);
  //      break;
  //
  //    case 12: // ----- Display 2nd roll digit
  //      Serial.print((abs(Serial_roll_buffer) / 100) % 10);
  //      break;
  //
  //    case 13: // ----- Display 3rd roll digit
  //      Serial.print((abs(Serial_roll_buffer) / 10) % 10);
  //      break;
  //
  ...
  //      Serial.print(abs(Serial_roll_buffer) % 10);
